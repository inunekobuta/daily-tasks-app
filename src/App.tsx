...
import PerformanceManagement from "./features/performance/PerformanceManagement";
...
export default function App() {
  ...
  const headerTitle = activeView === "tasks" ? "1日のタスク管理" : "業績管理";
  const headerSubtitle = activeView === "tasks" ? "Google連携・クラウド同期" : "売上・コストの記録";
  const toggleLabel = activeView === "tasks" ? "業績管理を見る" : "タスク管理に戻る";
    ...
    <div className="min-h-screen bg-[radial-gradient(1200px_800px_at_10%_-10%,#eef2ff,transparent),radial-gradient(1000px_600px_at_110%_10%,#fdf2f8,transparent),linear-gradient(to_bottom,#ffffff,70%,#f8fafc)] text-slate-900">
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200/70 bg-white/70 backdrop-blur-xl">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-bold tracking-tight">{headerTitle}</h1>
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              onClick={() => setActiveView((v) => (v === "tasks" ? "performance" : "tasks"))}
            >
              {toggleLabel}
            </Button>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        {activeView === "performance" ? (
          <PerformanceManagement supabase={supabase} userId={user.id} />
        ) : (
          <>
        {/* フィルタ */}
